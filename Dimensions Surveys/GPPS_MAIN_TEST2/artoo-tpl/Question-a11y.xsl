...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fo="http://www.w3.org/1999/XSL/Format">
                ...
                <xsl:choose>
                    ...
                    <xsl:when test="name() = 'Error'">
                        <xsl:apply-templates select=".">
							...
							<!-- added a new parameter in order to pass on the DIM error name -->
							<xsl:with-param name="errorName" select="@Name"/>
                        </xsl:apply-templates>
                    </xsl:when>
    ...
    <xsl:template match="Label | Error">
        ...
        <xsl:param name="bWithinTable" select="false()"/>
		<!-- added a new parameter in order to pass on the DIM error name -->
		<xsl:param name="errorName" select="@Name"/>
    ...
    <xsl:template name="LabelBase">
		...
		<!-- added a new parameter in order to pass on the DIM error name -->
		<xsl:param name="errorName" select="@Name"/>
		<!-- added the condition  and $sLabelClass!='mrErrorText' so that the empty div is not generated for inline error messages; this will make the error text display inline with the answer label but no matter as errors will be hidden via JS anyway -->
        <xsl:if test="Style/@ElementAlign = 'NewLine' and $sLabelClass!='mrErrorText'">
            <div></div>
        </xsl:if>
        <xsl:if test="$sLabelClass!='mrBannerText'">
            <xsl:element name="span">
                ...
                <xsl:if test="$sLabelClass='mrErrorText'">
                    <xsl:attribute name="data-error"><xsl:value-of select="$errorName"/></xsl:attribute>
                </xsl:if>				
                <xsl:attribute name="style">
...
<!--- TABLE -->
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                <!--- div tag needed for horizontal alignment of tables-->
				 <!--- get rid of the additional DIV element -->
                <!--<xsl:element name="div">-->

                    <!---<xsl:attribute name="class">
                        <xsl:text>table-responsive</xsl:text>
                    </xsl:attribute>-->
                    <!--- get rid of the table-responsive class, as it does not bring value, on the contrary -->
                    <xsl:element name="table">
                        <!---<xsl:if test="@Summary">
                         ...
                         <!--- get rid of the summary attribute in tables  -->
                        <xsl:if test="$bIncludeCSSStyles">
                            <xsl:attribute name="class">mrQuestionTable table table-bordered</xsl:attribute>
                        </xsl:if>
							<xsl:for-each select="parent::node()">
								<xsl:attribute name="style">
							...
							<!-- N.B. added tbody element wrapping all table rows; browsers add this if missing but thought it's best to sort it out here  -->	
    ...
    <xsl:template match="Cell">
        <xsl:choose>
		<!-- N.B. changed condition to generate table heading elements instead of TDs for mrGridCategoryText (statement cell) and mrGridQuestionText (field cell) -->
          <xsl:when test="string(@Class) = 'mrGridQuestionText' or string(@Class) = 'mrGridCategoryText' or @IsHeading"><!-- @IsHeading --> 
            <xsl:element name="th">
    ...
    <xsl:template name="CellImpl">
	  ...
	  <!-- wrap the contents of the TD element into a new div -->
	  <xsl:element name="div">
			<!-- assign the div a class of mrGridRadioCheckbox to allow for styling of inputs, but only when the cell has these two controls as descendants -->
			<xsl:if test="string(@Y) != '0' and string(@X) != '0' and (Control[1]/@Type = 'RadioButton' or Control[1]/@Type = 'CheckButton')">
				<xsl:attribute name="class">mrGridRadioCheckbox</xsl:attribute>
			</xsl:if>					
		  <xsl:for-each select="*">
			<xsl:choose>
			  <xsl:when test="name() = 'Control'">
            ...
            <xsl:element name="div">
                ...
                <xsl:if test="$bIncludeElementIds">
                    <xsl:attribute name="id">
                        ...
                        <xsl:text>.</xsl:text>
                        <xsl:value-of select="@Y"/>
						<!--- Append the ElementID value to the Cell so that we achieve unique Cell ids at the survey level for categorical answer options -->
						<xsl:value-of select="Control/@ElementID"/>						
                    </xsl:attribute>
                </xsl:if>
                <xsl:attribute name="class">
                    <xsl:text>mrFormRadioCheckbox</xsl:text>
                </xsl:attribute>
                <!--- CSS Class -->
                <xsl:if test="$bIncludeCSSStyles and @Class">
                    <xsl:attribute name="class">
...
<xsl:template name="StaticControl">
        <xsl:element name="span"> <!--- N.B. IIS changed this element to legend -->
            <xsl:attribute name="style">
                <xsl:if test="Style/@Width or Style/@Height">
                    <xsl:call-template name="SpanStyle"/>
                </xsl:if>
    ...
    <xsl:template name="SingleLineEditControl">
        ...
        <xsl:element name="input">
            ...
            <!--- IIS edits start here -->
            <xsl:if test="string(//@Y) = '0'">
                <xsl:if test="contains(@ElementID, 'O')">
                    <xsl:attribute name="aria-labelledby">
                        <xsl:value-of select="translate( @ElementID, 'O', 'C')"/>
                    </xsl:attribute>
                </xsl:if>   
            </xsl:if>
            
            <xsl:if test="//@Orientation = 'Column' ">
                <xsl:choose>
                    <xsl:when test="//Questions/Question/Table/Row[last()]/Cell[last()]/@Y = '1'">
                        <xsl:attribute name="aria-labelledby">
                            <xsl:text>Cell.</xsl:text>
                            <xsl:value-of select="../../@X"/>
                            <xsl:text>.0</xsl:text>     
                    ...
                    <xsl:otherwise>
                        <xsl:if test="number(//Questions/Question/Table/Row[last()]/Cell[last()]/@Y) > 1">
                            <xsl:attribute name="aria-labelledby">
                                <xsl:text>Cell.</xsl:text>
                                <xsl:value-of select="../../@X"/>
                                <xsl:text>.0 </xsl:text>
                                <xsl:text>Cell.0.</xsl:text>
                                <xsl:value-of select="../../@Y"/>
                            </xsl:attribute>
                        </xsl:if>   
                    </xsl:otherwise>
            ...
            <xsl:if test="string-length(//@Orientation)=0 ">
                <xsl:choose>
                    <xsl:when test="//Questions/Question/Table/Row[last()]/Cell[last()]/@X = '1'">
                        <xsl:attribute name="aria-labelledby">
                                <xsl:text>Cell.0.</xsl:text>
                                <xsl:value-of select="../../@Y"/>
                        </xsl:attribute>
                    ...
                    <xsl:otherwise>
                        <xsl:if test="number(//Questions/Question/Table/Row[last()]/Cell[last()]/@X) > 1">
                            <xsl:attribute name="aria-labelledby">
                                <xsl:text>Cell.0.</xsl:text>
                                <xsl:value-of select="../../@Y"/>
                                <xsl:text> </xsl:text>
    ...
    <xsl:template name="MultiLineEditControl">
        ...
        <xsl:element name="textarea">
            ...
            <!--- CSS Class -->
            <xsl:if test="$bIncludeCSSStyles">
                <xsl:attribute name="class">mrEdit mrEditTextarea</xsl:attribute>
            </xsl:if>
            <!--- Show Only -->
            <xsl:if test="$bShowOnly != false()">
                <xsl:attribute name="disabled"/>
			...
			<!--- data max length; added this attribute so that we can pass on the max no of chars that will be accepted as the answer -->
			<xsl:if test="@Length">
				<xsl:attribute name="data-maxlength"><xsl:value-of select="@Length"/></xsl:attribute>
			</xsl:if>
            <!--- Default text -->
            <xsl:choose>
                <xsl:when test="Category[1]/@Checked = 'true'">
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:element name="select">
                    ...
                    <xsl:if test="string(//@Y) = '0' and string-length(//@Y) != 0">
                        <xsl:if test="//@Orientation = 'Column' ">
                        <xsl:choose>
                            <xsl:when test="//Questions/Question/Table/Row[last()]/Cell[last()]/@Y = '1'">
                                <xsl:attribute name="aria-labelledby">
    ...
    <xsl:template name="RadioButtonControl">
        ...
        <xsl:element name="input">
			...
			<!-- if grid -->
			<xsl:if test="string(//@Y) != '0' or string(//@X) != '0'">
				<xsl:if test="//@Orientation = 'Column' ">
					<!-- grid per column -->
					<xsl:attribute name="aria-labelledby">
						<xsl:text>Cell.</xsl:text>
		...
		<xsl:choose>				
			<xsl:when test="$bIncludeElementIds and Category[1]/Label">	
				<!--- generates a label element for the radiobutton  -->			
				<xsl:element name="label">
					<xsl:attribute name="for">
						<xsl:value-of select="@ElementID"/>
						<xsl:if test="Category[1]/@CategoryID">
		...
		<!-- N.B. added the below condition to generate label elements following radiobutton inputs in grid cells that don't represent table headings TH -->
		<xsl:if test="//Table and (string(//@Y) != '0' or string(//@X) != '0')">
				<xsl:element name="label">
					<xsl:attribute name="for">
						<xsl:value-of select="@ElementID"/>
						<xsl:if test="Category[1]/@CategoryID">
    ...
    <!--- Control Label -->
        <xsl:element name="input">
			...
			<xsl:if test="string(//@Y) != '0' or string(//@X) != '0'">
				...
				<xsl:if test="string-length(//@Orientation)=0 ">
				<!-- grid per row -->
					<xsl:if test="not(contains(@ElementID, '_X'))">
						<xsl:attribute name="aria-labelledby">
							<xsl:text>Cell.0.</xsl:text>
								<xsl:value-of select="../@Y"/>
								<xsl:text> </xsl:text>
            ...
            <!--- flag exclusive answer options by applying a specific attribute to the input -->
            <xsl:if test="string(Style/Cell/@ColSpan) = '1'">
                <xsl:attribute name="data-exclusive">true</xsl:attribute>
            </xsl:if>			
        </xsl:element>
		<xsl:choose>
			<xsl:when test="$bIncludeElementIds and Category[1]/Label">
				<!--- generates a label element for the checkbox  -->
				<xsl:element name="label">
					<xsl:attribute name="for">
						<xsl:value-of select="@ElementID"/>
						<xsl:if test="Category[1]/@CategoryID">
		...
		<!-- N.B. added the below condition to generate label elements following checkbox inputs in grid cells that don't represent table headings TH -->
		<xsl:if test="//Table and (string(//@Y) != '0' or string(//@X) != '0')">
				<xsl:element name="label">
					<xsl:attribute name="for">
						<xsl:value-of select="@ElementID"/>
...
<!--- Style Templates -->
    ...
    <xsl:template name="LabelStyle">
        ...
        <xsl:if test="Style/@Hidden = 'true'">visibility: hidden;display: none;</xsl:if>
