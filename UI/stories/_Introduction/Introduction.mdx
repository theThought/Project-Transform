...
- Fonts:
    - Do we need so many font variants, or just WOFF?
    - Do we need to fetch from Google too, or just use local fonts?
    - Currently, only a subset of the WOFF fonts have been defined in the new CSS.
- `build` artefacts:
    - Currently in `.gitignore`, but they should probably be committed to repo so compiled CSS/JS (and maps) can be referenced in Survey?
- Images and favicons:
    - Do we need to copy them to `build` folder too?
